...
you may need: sudo apt install docker-compose
```
...
Make sure your `.env` file contains at least these variables:
...
Adjust or add other variables in `.env` as needed. To save the file, press CTRL+X, then Y, and then ENTER.
...
c. Then click on "Add rule" under "IPv4 Firewall", enter `5173` as your port, and hit "Create". Repeat the process for port `7091`.
