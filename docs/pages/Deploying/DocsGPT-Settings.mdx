...
- **`API_KEY`**:  Required for most cloud-based LLM providers. This is your authentication key to access the LLM provider's API. 
    - **Note:** When using `LLM_PROVIDER=docsgpt`, DocsGPT uses a default public API key (`sk-docsgpt-public`) and base URL (`https://oai.arc53.com`); in this case, you do not need to supply your own API key.
