...
- **API Call:** Alternatively, you can use the `/api/create_agent` endpoint to create a new agent, which returns an identifier and its associated API key. For detailed instructions, visit [DocsGPT API Documentation](https://gptcloud.arc53.com/).
...
- **ID:** A unique identifier for your API key. You can view this by making a call to `/api/get_agents` or inspecting the agent details.
